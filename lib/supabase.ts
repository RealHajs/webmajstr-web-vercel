...
export async function getDashboardStats() {
  const [partnersResult, newsResult, contactsResult, servicesResult] = await Promise.all([
  ...
  const [recentPartners, recentNews, recentContacts] = await Promise.all([
    supabase.from('partners').select('id, name, created_at').order('created_at', { ascending: false }).limit(5),
    supabase.from('news').select('id, title, published_at').order('published_at', { ascending: false }).limit(5),
    supabase.from('contacts').select('id, name, email, subject, created_at').order('created_at', { ascending: false }).limit(5)
  ])
